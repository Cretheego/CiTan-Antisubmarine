...
for n = 1:h:N
    ...
    while(1)
        ...
        %与边界发生碰撞
        if x_t(t+1) < 0 || x_t(t+1) > W
            x_t(t+1) = x_t(t+1) - V*cos(H0)*t;
            H0 = 2*pi - H0;
        end
        if y_t(t+1) < 0 || y_t(t+1) > H
...
%         x_st(2,t+1) = (W-V_s*t_T)*(t_T>0 & t_T<=t1) + 0*(t_T>t1& t_T<=t2) + (W-V_s)*(t3-t_T)*(t_T>t2 & t_T<=t3) + 0*(t_T>t3& t_T<=T) ;
%         y_st(2,t+1) = y_st(1,t+1) ;
%         
%         x_st(3,t+1) =  x_st(1,t+1);
%         y_st(3,t+1) = (H-(W_m/2 + floor(t/T)) *W_m)*(t_T>0 & t_T<=t2) + (H-(3*W_m/2 + floor(t/T) *W_m))*(t_T>t2 & t_T<=T);
%         
%         x_st(4,t+1) = x_st(2,t+1);
...
%             %Continue;
%         end
        d_fq = sqrt((x_st(1,t+1) - x_t(t+1))^2 + (y_st(1,t+1) - y_t(t+1))^2);
        if d_fq <= W_m/2
            find_cnt(n) = find_cnt(n)+1;
            %Continue;
        end
